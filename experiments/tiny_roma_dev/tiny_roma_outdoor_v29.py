...
resolutions = {"low":(448, 448), "medium":(14*8*5, 14*8*5), "high":(14*8*6, 14*8*6), "super_high": (768, 1024)}
...
class XFeatModel(nn.Module):
    ...
    def pos_embed(self, corr_volume):
        ...
        corrs_forward, inds_forward = corr_volume.reshape(B,H0,W0, H1*W1).max(dim=-1) # B, HW, H, W
        corrs_backward, inds_backward = corr_volume.reshape(B, H0*W0, H1,W1).max(dim=1)
    ...
    def corr_volume(self, feat0, feat1):
        ...
        corr_volume = torch.einsum('bcyx,bcvu->byxvu', feat0, feat1)
