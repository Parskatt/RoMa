...
class XFeatModel(nn.Module):
    ...
    def __init__(self, xfeat = None, freeze_xfeat = True, sample_mode = "threshold_balanced", symmetric = True):
        ...
        match_dim = 256
        self.coarse_refiner = nn.Sequential(
        ...
        self.more_fusion = nn.Sequential(
    ...
    def pos_embed(self, corr_volume):
        ...
        grid0 = torch.stack(
            torch.meshgrid(
                ...
                indexing = "xy"), 
            dim = -1).float().cuda().reshape(H0*W0, 2)
        corrs_forward, inds_forward = corr_volume.reshape(B,H1*W1, H0,W0).max(dim=1) # B, HW, H, W
        corrs_backward, inds_backward = corr_volume.reshape(B, H1,W1, H0*W0).max(dim=-1)
        pos_forward = grid1[inds_forward.view(-1)].reshape(*corrs_forward.shape, 2).permute(0,3,1,2)
        pos_backward = grid0[inds_backward.view(-1)].reshape(*corrs_backward.shape, 2).permute(0,3,1,2)
        forward_backward = F.grid_sample(
    ...
    def corr_volume(self, feat0, feat1):
        ...
        corr_volume = torch.einsum('bcyx,bcvu->bvuyx', feat0, feat1)
    ...
    def match(self, im0, im1, *args, batched = True):
        ...
        flow, certainty = corresps[4]["flow"], corresps[4]["certainty"]
        #flow, certainty = corresps[8]["gm_flow"], corresps[8]["gm_certainty"]
        if True:
            B,C,H,W = im0.shape
            flow = F.interpolate(
                flow, 
                size = (H,W), 
                mode = "bilinear", align_corners = False).permute(0,2,3,1).reshape(B,H,W,2)
    ...
    def forward(self, batch):
        ...
        feats_x0_c = self.more_fusion(feats_x0_c)
        feats_x1_c = self.more_fusion(feats_x1_c)
        B, C, H0, W0 = im0.shape
        B, C, H1, W1 = im1.shape
        to_normalized = torch.tensor((2/W1, 2/H1, 1)).to(im0.device)[None,:,None,None]
        # corr volume + match highest sim
        corr_volume = self.corr_volume(feats_x0_c, feats_x1_c)
        coarse_matches = self.pos_embed(corr_volume)
        coarse_warp = coarse_matches[:,:2]
        #coarse_matches = torch.cat((coarse_warp, torch.zeros_like(coarse_warp[:,-1:])), dim=1)
        corresps[8] = {"corr_volume": corr_volume, "gm_flow": coarse_warp, "gm_certainty": coarse_matches[:,2:]}
        
        # backwarp im1 feats
        feats_x1_c_warped = F.grid_sample(feats_x1_c, coarse_matches.permute(0, 2, 3, 1)[...,:2], mode = 'bilinear', align_corners = False)
        # refine im0 + backwarp + coarse_matches
        coarse_matches_delta = self.coarse_refiner(torch.cat((feats_x0_c, feats_x1_c_warped, coarse_warp), dim=1))
        # these are in pixel coordinates, convert to normalized, and add to initial
        coarse_matches = coarse_matches + coarse_matches_delta*to_normalized
        corresps[8].update({"flow": coarse_matches[:,:2], "certainty": coarse_matches[:,2:]})
        
        # upsample, detach
        coarse_matches_up = F.interpolate(coarse_matches, size = feats_x0_f[0].shape[-2:], mode = "bilinear", align_corners = False)        
        coarse_matches_up_detach = coarse_matches_up.detach()#note the detach
        
        # backwarp fine im1 feats
        feats_x1_f_warped = F.grid_sample(feats_x1_f, coarse_matches_up_detach.permute(0, 2, 3, 1)[...,:2], mode = 'bilinear', align_corners = False)
        # refine
        fine_matches_delta = self.fine_refiner(torch.cat((feats_x0_f, feats_x1_f_warped, coarse_matches_up_detach[:,:2]), dim=1))
        # convert to normalized and add 
...
if __name__ == "__main__":
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = XFeatModel(freeze_xfeat=True).to(device)
