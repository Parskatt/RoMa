...
class XFeatModel(nn.Module):
    ...
    def __init__(self, xfeat = None, freeze_xfeat = True):
        ...
        pos_dim = 32
        decode_dim = 128
        self.pos_encoder = nn.Linear(2, pos_dim//2, bias = False)
        self.warp_decoder = nn.Sequential(
            BasicLayer(pos_dim, decode_dim, kernel_size =1, padding = 0,),
            BasicLayer(decode_dim, decode_dim, kernel_size =1, padding = 0,),
            BasicLayer(decode_dim, decode_dim, kernel_size =1, padding = 0,),
            nn.Conv2d(decode_dim, 2, kernel_size =1, padding = 0,),)
        match_dim = 256
    ...
    def pos_embed(self, corr_volume):
        ...
        grid = 2 * math.pi * self.pos_encoder(grid[None])[0]
        grid = torch.cat((grid.sin(), grid.cos()), dim = -1)
    ...
    def forward(self, batch):
        ...
        coarse_emb_warp = self.pos_embed(corr_volume)
        coarse_warp = self.warp_decoder(coarse_emb_warp)
...
def train(args):
    ...
    depth_loss = RobustLosses(
        ...
        c = 1e-4,)
    parameters = [
        {"params": model.parameters(), "lr": roma.STEP_SIZE * 5e-4 / 8},
    ]
    optimizer = torch.optim.AdamW(parameters, weight_decay=1e-6)
